...
- A teljes mezőréteget egy bal alsó, 40×40 px-es hatszög gomb kapcsolja ki és be. Bekapcsolva lila; kikapcsolva szürke háttér és a hexagon előtt futó halványpiros áthúzás jelzi a rejtett állapotot. Közvetlenül fölötte csak a rögzítési oldalon jelenik meg az egybefüggő `+ / −` zoomvezérlő. A gombos zoom nem szakítja meg a pozíciókövetést; a kézi húzás és csippentés továbbra is igen. A megjegyzett 3D nézet már a térkép első betöltésekor érvényesül; az ikon és a tényleges kameradőlés nem térhet el. A kiválasztott küldetés vezetővonala szaggatott `#FA5F73`.
...
többi támogatott böngésző FCM Web Push/VAPID tokent használ. A natív iOS és
Android app Firebase Messagingen keresztül FCM tokent regisztrál; a Firebase
iOS-en APNs-en, Androidon közvetlenül FCM-en kézbesít. Minden token a
`devices/{uid}/tokens/{token}` alatt él `web` / `ios` / `android` platform-
