...
| 2 | **Hosszú rögzítés stabilitása** | sok km / hosszú idő alatt se lassuljon, ne akadjon | **részben mérve, gyökérok azonosítva** — 100×/1000×-es gyorsított lejátszásnál a wallclock ×2,6-ra nőtt (609,8 s / 236,2 s) a **térképrajzolás** miatt (A/B igazolva), NEM a worker-oldali preview miatt. **VALÓS (1×) sebességnél viszont a Game Loop scenario 4 (ramp) hiba nélkül lefutott**, már 17 hurkos/14 799 cellás állapotban is — a főszál végig tiszta maradt. Lásd alább, „Mapbox-rétegrajzolás". **Hiányzik:** a térképrajzolás önálló mérőszáma (ma A/B megfigyelés + a ramp-scenario sikeres lefutása, nem számszerűsített GPU-metrika). |
...
## Eszköz a folytatáshoz, és az első futása
...
**Első futás eredménye (2026-09-05, Samsung SM-G780F):** hiba nélkül
befejeződött (`error: null`), ugyanaz a végeredmény, mint a többi scenario-2
futásnál (26 544 m, 17 hurok, 14 799 cella, 496 GP). A valós (1×) szakaszban,
**MÁR 17 hurkos, 14 000+ cellás állapotban**, a `preview.total` (worker szál):
átlag **16,4 ms**, p95 **68,8 ms**, max **194,7 ms** — magas, de a főszál
végig tiszta maradt (nincs UI-blokkolás, nincs akadás). Ez megerősíti: **a
korábban tapasztalt brutális akadás a mesterséges 100×/1000×-es gyorsításból
jött** (a térképrajzolás a valóságban lehetetlen ütemben kapott
frissítéseket), **nem a valós idejű rögzítés viselkedéséből**.
